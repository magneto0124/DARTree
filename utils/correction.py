...
class DominoCorrectionScorer:
    def __init__(
        ...
        self.gru_b_ih = (
            gru.bias_ih_l0.detach().contiguous() if gru.bias else None
        )
        self.gru_b_hh = (
            gru.bias_hh_l0.detach().contiguous() if gru.bias else None
        )
    ...
    def candidate_topk_from_precomputed(
        self,
        z_proj: torch.Tensor,
        ...
        *,
        ...
        candidate_ids = candidate_ids.view(-1).long()
        candidate_base_vals = candidate_base_vals.view(-1)
        candidate_count = int(candidate_ids.numel())
        if candidate_count <= 0:
            raise ValueError("candidate_ids must be non-empty")

        k = max(1, min(int(k), candidate_count))
        s_proj = F.linear(h_state, self.w_s, None)
        mid = self.middle((z_proj + s_proj).unsqueeze(1)).squeeze(1)
        bias = mid @ candidate_weight.t()
        if candidate_bias is not None:
            bias = bias + candidate_bias.view(1, -1)

        candidate_logits = candidate_base_vals.view(1, -1).to(bias.dtype) + bias
        candidate_logits_f = candidate_logits.float()
        expanded_ids = candidate_ids.view(1, -1).expand(
            candidate_logits.shape[0], -1
        )
        if k >= candidate_count and not sort_result:
            top_vals = candidate_logits_f
            top_ids = expanded_ids
        else:
            top_vals, top_pos = torch.topk(candidate_logits_f, k=k, dim=-1)
            top_ids = expanded_ids.gather(1, top_pos)
        log_z = (
            torch.logsumexp(candidate_logits_f, dim=-1, keepdim=True)
            if compute_log_z
            else torch.zeros(
                (candidate_logits.shape[0], 1),
                dtype=torch.float32,
        ...
        return top_vals, top_ids, log_z

    def update_hidden(
        self, token_ids: torch.Tensor, h_state: torch.Tensor
    ) -> torch.Tensor:
        token_ids = token_ids.view(-1)
        gi = self._gru_input_proj_table[token_ids]
        gh = F.linear(h_state, self.gru_w_hh, self.gru_b_hh)
        if self._can_triton_fuse:
            h_out = torch.empty_like(h_state)
            self._fused_gru_cell(gi, gh, h_state, h_out)
            return h_out
...
class DraftCorrectionGraphRunner:
    ...
    def _forward_impl(self):
        ...
        for t in range(self.steps):
            ...
            if t + 1 < self.steps:
                # Gather the precomputed GRU input projection.
