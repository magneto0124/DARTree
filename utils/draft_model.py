...
class Qwen3DFlashAttention(nn.Module):
    """Multi-head attention used by the diffusion drafter."""
...
class DFlashDraftModel(Qwen3PreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        if not is_domino_projector(self.projector_type):
            raise ValueError(
                "DARTree only supports Domino checkpoints; "
    ...
    def spec_generate(
        ...
        *accepted draft chain* (the contiguous run of draft tokens the target
        ...
        stage_times: dict[str, float] = {"draft": 0.0, "verify": 0.0, "commit": 0.0}
        draft_prefill = True
        ...
        while start < max_length:
            ...
            if block_size > 1:
                draft_stage_start = cuda_time(self.device)
                if not is_domino_projector(self.projector_type):
                    raise ValueError(
                        "DARTree only supports Domino checkpoints; "
                        f"got projector_type={self.projector_type!r}."
                ...
                if draft_prefill:
                    ...
                    stage_times["draft"] += cuda_time(self.device) - draft_stage_start

            verify_stage_start = cuda_time(self.device)
            output = target(
                verify_ids,
            ...
            stage_times["verify"] += cuda_time(self.device) - verify_stage_start

            commit_stage_start = cuda_time(self.device)
            ...
            stage_times["commit"] += cuda_time(self.device) - commit_stage_start
